...
<div class="p-6">
    ...
    <form method="POST" action="{{ route('staff.update', $staff->id) }}" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="position" value="{{ old('position', $staff->position) }}" class="w-full border rounded p-2">
        </div>

        <div>
            <label class="block mb-1 font-medium">Phone Number</label>
            <input type="text" name="phone_num" value="{{ old('phone_num', $staff->phone_num) }}" class="w-full border rounded p-2">
            @error('phone_num') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
        </div>
