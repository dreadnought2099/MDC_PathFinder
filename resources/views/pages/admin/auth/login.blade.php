    ...
    <div class="min-h-screen flex items-center justify-center px-4 sm:px-6 lg:px-8 py-8">
        <div
                ...
                <form action="{{ route('login') }}" method="POST" class="space-y-4 sm:space-y-6">
                    ...
                    <div x-data="{ show: false }" class="relative">
                        ...
                        <button type="button" @click="show = !show"
                            class="absolute right-2 sm:right-3 top-1/2 transform -translate-y-1/2 hover:text-primary focus:primary focus:outline-none focus:ring-0">
                            <svg x-show="!show" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5" fill="none"
            ...
            <div
                ...
                <img src="https://cdn.jsdelivr.net/gh/dreadnought2099/MDC_PathFinder/public/images/mdc.png" class="w-24 h-24 sm:w-32 sm:h-32 md:w-36 md:h-36 rounded-full" alt="MDC Logo"/>
                <h3 class="text-base sm:text-lg text-center mt-3 sm:mt-4 px-4">{{ config('app.name') }} <br>Management System</h3>
            </div>
        </div>
    </div>
@endsection
