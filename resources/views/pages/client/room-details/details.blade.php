<div
        ...
        <section
            ...
            <div
                ...
                <div class="space-y-3 sm:space-y-4 max-w-3xl pointer-events-auto">
                    <h1
                        class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-extrabold drop-shadow-2xl leading-tight">
                        {{ $room->name }}
                    </h1>
                    ...
                    @if ($room->description)
                        <div class="relative max-w-3xl">
                            ...
                            <button id="toggleDescriptionBtn"
                                class="mt-2 text-xs sm:text-sm md:text-base text-primary font-semibold hover-underline focus:outline-none">
                                See more
                            </button>
                        </div>
                    @endif
    ...
    @if ($room->video_path)
        <div>
            <h2 class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-gray-200 mb-4 text-center">Room Video</h2>
    ...
    @if ($room->staff->isNotEmpty())
        <div>
            <h2 class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-gray-200 mb-4 text-center">Assigned Staff
            </h2>
                    ...
                    <div
                        ...
                        <div class="cursor-pointer overflow-hidden"
                            ...
                            <img src="{{ $member->photo_path ? Storage::url($member->photo_path) : asset('images/mdc.png') }}"
                                ...
                                class="w-full h-40 sm:h-56 object-cover group-hover:scale-110 transition-transform duration-500">
                        </div>
                        <div class="p-4 sm:p-6 text-center">
                            <a href="{{ route('staff.client-show', $member->token) }}"
                                class="block text-base sm:text-lg font-bold text-slate-800 dark:text-gray-200 hover:text-primary transition-colors mb-1"
    ...
    <!-- Office Hours -->
    <div class="mt-6">
        <h3 class="text-2xl sm:text-3xl font-bold text-slate-800 mb-2 text-center dark:text-gray-300">Office Hours</h3>
                ...
                <div class="space-y-3">
                    @foreach ($room->grouped_office_hours as $timeRange => $days)
                        <div class="flex flex-col sm:flex-row sm:items-center gap-2">
                            <div
                                class="font-medium text-xs sm:text-sm md:text-base text-gray-800 min-w-0 flex-1 dark:text-gray-300">
    ...
    <div class="text-center pt-8 border-t border-gray-200 dark:border-gray-600">
        <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-300 mb-3">Want to explore another room?</p>
        <a href="{{ route('scan.index') }}"
            class="inline-block text-xs sm:text-sm md:text-base bg-primary hover:bg-white hover:text-primary text-white py-2 sm:py-3 px-4 sm:px-6 rounded-lg border border-primary transition-all duration-300 dark:hover:bg-gray-800 shadow-primary-hover">
...
<div id="imageModal"
    ...
    <div class="absolute top-5 right-5 flex items-center space-x-4 sm:space-x-8">
        ...
        <a id="downloadBtn" href="#" download title="Download Image"
            class="p-2 rounded-xl transition-all hover:scale-120 ease-in-out duration-300">
        ...
        <button onclick="closeModal()"
            class="p-2 rounded-xl transition-all hover:scale-120 ease-in-out duration-300 cursor-pointer"
            title="Close Modal">
