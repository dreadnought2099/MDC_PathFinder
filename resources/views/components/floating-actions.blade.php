<div>
    ...
    <div x-data="{ open: false }" @click.away="open = false"
        ...
        <!-- Show for Admin or Office Manager with create staff permission -->
        @if (auth()->user()->hasRole('Admin') ||
                (auth()->user()->hasRole('Office Manager') && auth()->user()->can('create staff')))
            <template x-if="open">
                <div
                    ...
                    @if (auth()->user()->hasRole('Admin'))
                        ...
                        <!-- Add Path Images (Admin Only) -->
                        @php
                            $selectedPathId = session('selectedPathId');
                            $path = $selectedPathId
                                ? \App\Models\Path::find($selectedPathId)
                                : \App\Models\Path::first();
                        @endphp
                    ...
                    <!-- Add Staff Member (Both Admin and Office Manager with permission) -->
                    @can('create staff')
                    ...
                    @if (auth()->user()->hasRole('Admin'))
                        <a href="{{ route('room-user.create') }}" class="group flex items-center space-x-2">
                            ...
                            <span class="text-sm text-gray-700 hover-underline dark:text-gray-300">Add Office
                                User</span>
                        </a>
                    @endif
